...
def event_detail(event_id):
    """Show a single event."""
    # TODO: Get the event with the given id and send to the 
    guests = Guest.query.all()
    given_id = request.form.get('event_id')
    this_event = Event.query.filter_by(id=given_id)
    return render_template('event_detail.html', this_event=this_event, guests=guests)
...
def rsvp(event_id):
    ...
    if is_returning_guest:
        ...
        this_guest = Guest.query.filter_by(name=guest_name)
...
def guest_detail(guest_id):
    ...
    this_guest = Guest.query.filter_by(id='guest_id')
